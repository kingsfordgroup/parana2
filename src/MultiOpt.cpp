...
namespace MultiOpt {
    ...
    costMapFunT getCostFunDict ( double cc, double dc, bool directed ) {
        ...
        costDict[ both ][ both ] = [=](const double& pf, const double& pr) { return make_tuple( undirected ? (1.0-pf)*cc : (1.0-pf)*dc + (1.0-pr)*dc, "n" ); };
    ...
    selfCostMapFunT getSelfLoopCostFunDict ( double cc, double dc, bool directed ) {
        ...
        selfLoopCostDict[ none ][ false ] = [=] (const double& p) { return make_tuple( p*dc, "n" ); };
        ...
        selfLoopCostDict[ both ][ true ] = [=] (const double& p) { return make_tuple( (1.0-p)*cc, "n" ); };
    ...
    unique_ptr<ForwardHypergraph>  buildSolutionSpaceGraph( const TreePtrT& t,
        ...
        cerr << "number of tree nodes = " << nodes.size() << "\n";
        size_t ctr1 = 0; size_t ctr2 = 0; size_t ctr3 = 0;
        ...
        cerr << "CTR1 = " << ctr1 << ", CTR2 = " << ctr2 << ", CTR3 = " << ctr3 << "\n";
    ...
    void leafCostDict( unique_ptr<ForwardHypergraph>& H, TreePtrT& T, GT& G, bool directed, double cc, double dc, slnDictT& slnDict ) {
        ...
        */
        ...
        for( auto n : leafHypernodes ) {
            ...
            if ( lostU || lostV ) {
                ...
                if (u != v) {
                    ...
                    cerr << T->getNodeName(nd.u()) << ", " << T->getNodeName(nd.v()) << "\n";
                    /*
                    ...
                    */
                    ...
                    if ( flip != "n" ) { nef += 1; effectiveEdges.insert( make_tuple(nd.u(),nd.v(),flip) ); }
                    vector<size_t> ev;
        ...
        double sum = 0.0;
        for( auto n : slnDict ) { sum += (n.second)[0].cost; }
        cerr << "SUM = " << sum << "\n";
        cerr << "NUM LOST = " << nlost << "\n";
    ...
    vector< tuple<double, cl_I> > countEdgeSolutions( 
        ...
        while ( !pq.empty() && numSln <= k ) {
            ...
            double cost; vector<size_t> inds;
            std::tie(cost,inds) = pq.front();
            std::pop_heap( pq.begin(), pq.end(), ord ); pq.pop_back();
            ...
            if ( edgeSlns.size() == 0 || (std::abs(cost - get<0>(edgeSlns.back())) > epsilon) ) {
                if (printMe) {
                    cerr << "IN IF: edgeSlns = ";
        ...
        if(edgeSlns.size() == k) { edgeSlns.pop_back(); }
        return edgeSlns;
    }
...
void viterbiCount( unique_ptr<ForwardHypergraph>& H, TreePtrT& t, TreeInfo& ti, double penalty, const vector<size_t>& order,
        ...
        for ( auto vit = order.begin(); vit != order.end(); ++vit, ++ctr ) {
            ...
            for ( const auto& e : H->incident(*vit) ) {
                ...
                if ( nameU > nameV ) { std::swap(nameU, nameV); }
                bool printMe = ( nameU == "129064_Fr" );// && nameV == "n9758" );
            ...
            std::sort(nedgesln.begin(), nedgesln.end(),
                []( const cl_I& x, const cl_I& y) { return x < y; }
            );
            for ( auto e : nedgesln ) {
                out << "\t" << e;
            }
